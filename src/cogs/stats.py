...
class Stats(commands.Cog):
    ...
    async def stats(
        ...
        if last is not None and last > 0:
            ...
            no_sq_scores = [
                e.get("score", 0) for e in table_events if e.get("tier") != "SQ"
            ]
            flat_partner = [
                p for e in table_events for p in (e.get("partnerScores") or [])
            ]
            ...
            fields = [
                ...
                (
                    "Avg. Score (No SQ)",
                    f"{sum(no_sq_scores) / len(no_sq_scores):.1f}"
                    if no_sq_scores
                    else "N/A",
                ),
                (
                    "Partner Avg.",
                    f"{sum(flat_partner) / len(flat_partner):.1f}"
                    if flat_partner
                    else "N/A",
                ),
                BLANK,
            ...
            fields = [
                ...
                (
                    "Last 10 Matches",
                    f"{player['winLossLastTen']}（{player['gainLossLastTen']}）",
                ),
                BLANK,
                ...
                (
                    "Partner Avg.",
                    f"{player['partnerAverage']:.1f}"
                    if "partnerAverage" in player
                    else "N/A",
                ),
            ]
        ...
        embed = discord.Embed(
            title=f"S{season} Stats{title_suffix} - {cfg.label(game_mode)}",
